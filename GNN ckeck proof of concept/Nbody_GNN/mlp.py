...
class mlp_rollout(nn.Module):
    def __init__(self,input_dim,hidden_dim,output_dim,acts,bias=True):
        ...
        MLP = mlp(input_dim,hidden_dim,output_dim,acts,bias)
        self.net = MLP.net

    def forward(self,t,y):
        if y.dim() == 3:#batch, more dim dataset
           
            self.trajectory = torch.Tensor(len(t),y.shape[0],1,y.shape[-1],)
            
            self.trajectory[0,:,:,:] = y.clone()
        else: # singledataset
           
            self.trajectory = torch.Tensor(len(t),1,y.shape[-1])
