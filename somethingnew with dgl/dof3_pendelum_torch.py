...
def eval_ham(trajectory):
    model =pendelum3dof(1,1,1,1,1,1,9.81)
    return dof3_hamiltonian_eval(model,trajectory)
...
def perfect_sample(model,trajectory):
    b = -torch.pi
    e = torch.pi
    print("angle analysis")
    for i in range(trajectory.shape[0]):
        temp = trajectory[i,0,0:3]
        list =[0,1,2]
        for i in list:
            if temp[i] > e or temp[i] < b:
                ...
                return False
    print("H analysis")
    H = dof3_hamiltonian_eval(model,trajectory)
    H_mean = torch.mean(H - H[0])
    H_std = torch.std(H)
    if H_mean < 1e-2 and H_std < 1e-3 :
        return True
    else:
        print(H_mean)
        print(H_std)
...
def dof3_dataset(model,t,y0,method="rk4"):
    ...
    for i in tqdm(range(y0.shape[0])):
        list.append(dof3_trajectory(model,t,y0[i,:,:],method).unsqueeze(1))
...
def angle_converter(trajectory):
    #print("converting angles")
    #print(trajectory.shape)
    for i in tqdm(range(trajectory.shape[0])):
        for a in [0,1,2]:
            temp = trajectory[i,0,a]
            temp = temp % (2*math.pi)
           # print(temp)
            if temp >= math.pi and temp <= 2*math.pi:
                #print("converted {} | {}".format(temp,math.pi))
                alpha = (temp-math.pi)/math.pi
                temp = -math.pi + alpha*math.pi
            ...
            if temp <= -math.pi and temp >= - 2*math.pi:
               # print("converted {} | {}".format(temp,-math.pi))
                ...
                #print("converted")
            with torch.no_grad():
                trajectory[i,0,a]=temp.detach().clone()    
    return trajectory
"""    
