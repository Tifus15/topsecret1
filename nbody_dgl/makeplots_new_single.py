...
model.load_state_dict(torch.load("server_6body_single.pth"))
model7 = rollout_GNN_GRU(g7,4,128,128,acts=["tanh",""],type="GCN")
model7.load_state_dict(torch.load("server_7body_single.pth"))
...
mod_x7_2 = model7(t,xe7[0,:,:])
fig = plt.figure()
plt.plot(t,H6[100,0:128])
plt.show()
fig = plt.figure()
plt.plot(t,H7[100,0:128])
plt.show()
...
for j in range(len(t)):
    ...
    for i in range(7): 
        ...
        axs[2,0].scatter(xe7[j,i,0].detach().numpy(),xe7[j,i,1].detach().numpy())
        axs[2,1].scatter(mod_x7_2[j,i,0].detach().numpy(),mod_x7_2[j,i,1].detach().numpy())
        
    axs[0,0].set_title("GROUND TRUTH 6 bodies")
    axs[0,1].set_title("AFTER TRAINING 6 bodies")
    axs[1,0].set_title("GROUND TRUTH 7 bodies")
    axs[1,1].set_title("AFTER TRAINING 6 bodies - 7 bodies")
    axs[2,0].set_title("GROUND TRUTH 7 bodies")
    axs[2,1].set_title("AFTER TRAINING 7 bodies")
