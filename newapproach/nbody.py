...
def simulate(
	...
	mass = N*np.ones((N,1))/N  # total mass of particles is 20
	pos  = np.random.randn(N,3)   # randomly selected positions and velocities on the sphere with radius 1.0
	...
	plt.savefig('nbody_'+str(N)+"_"+date+'.png',dpi=240)
	plt.show()
	print(KE_save+PE_save)
	#saving data DA
	
	#date = now.strftime("%m%d%Y%H%M%S")
	with open('nbody_'+str(N)+"_"+date+'.npy', 'wb') as f:
		np.save(f, save_pos)
		np.save(f, save_vel)
