...
class rollout_GNN_GRU_inverted(nn.Module):
    def __init__(self,g,in_dim,hid,gnn_size,acts,bias=True,type="GCN"):
        super().__init__()
        self.GRU = nn.GRU(hid,in_dim,1)
        self.GNN = GNN_maker(g,in_dim,gnn_size,hid,acts,bias=bias,type=type)
        self.g = g
    ...
    def forward(self,t,x0):
        ...
        output[0,:,:] = x0
        xi = self.GNN(output[0,:,:].squeeze())
        xii, hidden = self.GRU(xi.unsqueeze(dim=0))
        ...
        for i in range(2,T):
            ...
            xii, hidden = self.GRU(xi.unsqueeze(dim=0),hidden)
            #print("hidden: {}".format(hidden.shape))
