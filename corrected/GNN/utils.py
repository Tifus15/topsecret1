...
def convert2dgl_snapshots(g_snaps,src,dst):
    ...
    for snap in g_snaps:
        ...
        temp.ndata["x"]= snap[:,:,0:int(whole/2)]
        temp.ndata["dx"] = snap[:,:,int(whole/2):whole]
        temp.ndata["H"] = snap[:,:,-1]
        graphs.append(temp)
    return graphs
...
def visualize_hamiltonian(H_ground, H,t):
    ...
    plt.plot(t,H_ground.detach().numpy(),c="b")
    plt.legend(["prediction","ground truth"])
    plt.show()  

def viz_traj(ground,model_eval,type,t=None):
    ground = ground.squeeze()
    model_eval = model_eval.squeeze()
    if type == "dof2":
        fig, ax = plt.subplots(2)
        ax[0].plot(model_eval[0,:,0].detach().numpy(),model_eval[0,:,1].detach().numpy())
        ax[0].plot(ground[0,:,0].detach().numpy(),ground[0,:,1].detach().numpy())
        ax[0].legend(["prediction","ground_truth"])
        ax[1].plot(model_eval[1,:,0].detach().numpy(),model_eval[1,:,1].detach().numpy())
        ax[1].plot(ground[1,:,0].detach().numpy(),ground[1,:,1].detach().numpy())
        ax[1].legend(["prediction","ground_truth"])
        
    if type == "dof3" and not isinstance(t,torch.Tensor):
        ...
        ax[1].plot(ground[1,:,0].detach().numpy(),ground[1,:,1].detach().numpy())
        ax[1].legend(["prediction","ground_truth"])
        ax[2].plot(model_eval[2,:,0].detach().numpy(),model_eval[2,:,1].detach().numpy())
        ax[2].plot(ground[2,:,0].detach().numpy(),ground[2,:,1].detach().numpy())
        ax[2].legend(["prediction","ground_truth"])
    
    else:
        fig, ax = plt.subplots(2,3)
        ax[0,0].set_title("joint1 q")
        ax[0,1].set_title("joint2 q")
        ax[0,2].set_title("joint3 q")
        ax[1,0].set_title("joint1 p")
        ax[1,1].set_title("joint2 p")
        ax[1,2].set_title("joint3 p")
        ax[0,0].plot(t.detach().numpy(),model_eval[0,:,0].detach().numpy())
        ax[0,0].plot(t.detach().numpy(),ground[0,:,0].detach().numpy())
        ax[0,0].legend(["prediction","ground_truth"])
        ax[0,0].set_xlabel("t")
        ax[0,0].set_ylabel("q")
        ax[0,1].plot(t.detach().numpy(),model_eval[1,:,0].detach().numpy())
        ax[0,1].plot(t.detach().numpy(),ground[1,:,0].detach().numpy())
        ax[0,1].legend(["prediction","ground_truth"])
        ax[0,1].set_xlabel("t")
        ax[0,1].set_ylabel("q")
        ax[0,2].plot(t.detach().numpy(),model_eval[2,:,0].detach().numpy())
        ax[0,2].plot(t.detach().numpy(),ground[2,:,0].detach().numpy())
        ax[0,2].legend(["prediction","ground_truth"])
        ax[0,2].set_xlabel("t")
        ax[0,2].set_ylabel("q")

        ax[1,0].plot(t.detach().numpy(),model_eval[0,:,1].detach().numpy())
        ax[1,0].plot(t.detach().numpy(),ground[0,:,1].detach().numpy())
        ax[1,0].legend(["prediction","ground_truth"])
        ax[1,0].set_xlabel("t")
        ax[1,0].set_ylabel("p")
        ax[1,1].plot(t.detach().numpy(),model_eval[1,:,1].detach().numpy())
        ax[1,1].plot(t.detach().numpy(),ground[1,:,1].detach().numpy())
        ax[1,1].legend(["prediction","ground_truth"])
        ax[1,1].set_xlabel("t")
        ax[1,1].set_ylabel("p")
        ax[1,2].plot(t.detach().numpy(),model_eval[2,:,1].detach().numpy())
        ax[1,2].plot(t.detach().numpy(),ground[2,:,1].detach().numpy())
