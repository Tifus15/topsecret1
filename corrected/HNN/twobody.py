...
class gradH_rconst(nn.Module):
    ...
    def forward(self,t,y):
        print(self.A.shape)
        print("shape of y in creator {}".format(y.shape))
...
class twobody:
    ...
    def make_dataset(self,points,samples,T=0,r1_range=[1,5]):
        ...
        for i in tqdm(range(samples)):
            ...
            print("twobody x: {}".format(temp.shape))
            print("calling function")
            dtemp = model(0,temp) # this works but on threbody dont, why?
            print("twobody dx: {}".format(dtemp.shape))
